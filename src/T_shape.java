...
interface T_shape{
  ...
  //Return the number corresponding to each shape
  int the_shape();
 ...
 //Rotate shape to left from point of rotation, return the rotated points, for user to check for boundries
  ...
  // return the coordinate of the squares
  // Use spawn position of grid as starting point/ reference, when 
  //creating coordinate of shape. Use the grid class to move shapes,
  // Just move the "current active" coordinate on grid accordingly
  // start col in 3-6 start row 0-3  (4x4 square)
  XY[] get_shape_cord();
  
  //Set the shape coordinates, if block was rotated, new coordinate should be updated using this
