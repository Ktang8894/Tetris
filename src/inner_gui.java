...
class inner_gui extends JPanel{
  ...
  private int line_level = 0;  // 10 or more -> increase level
  ...
  private void spawn_tetris(){
    ...
    if(game_over == false){
      ...
      //Update score if at least one line is cleared
      ...
      for(int i=0; i<tt.length; i++){
        ...
        if(squares[tt[i].y_c][tt[i].x_c].in_use == true){
          game_over = true;
          game_timer.stop();
          clock_timer.stop();
          action_ok = false;
      ...
      if(game_over == false){
        //Initialize in_use state
        for(int i=0; i<tt.length; i++){
          squares[tt[i].y_c][tt[i].x_c].in_use = false;
        }
  ...
  private void add_labels(int width, int height){
    for(int i=0; i < height; i++){
      for(int j=0; j < width; j++){
        ...
        squares[i][j].in_use = false;  //Tetromino that is at the bottom -> static block (true)
    ...
    public void actionPerformed(ActionEvent event){
      ...
      for(int i = 0; i<temp_pos.length; i++){
        ...
        if(temp_pos[i].y_c + 1 > 19){
          //Set tetrominos to be in_use
          for(i = 0; i<temp_pos.length; i++){
            squares[temp_pos[i].y_c][temp_pos[i].x_c].in_use = true;
          }
        ...
        if(squares[temp_pos[i].y_c + 1][temp_pos[i].x_c].in_use == true){
          //Set tetrominos to be in_use
          for(i = 0; i<temp_pos.length; i++){
            squares[temp_pos[i].y_c][temp_pos[i].x_c].in_use = true;
          }
    ...
    public void keyPressed(KeyEvent e){
      ...
      if(e.getKeyCode() == KeyEvent.VK_LEFT){
        ...
        if(action_ok == true){
          ...
          for(int i=0; i<shape_pos.length; i++){
            //If collided
            if(squares[shape_pos[i].y_c][shape_pos[i].x_c-1].in_use == true){
              temp_state = false;
              game_timer.start();
              return;
            }
          ...
          if(temp_state == true){
            ...
            for(int i=0; i<shape_pos.length; i++){
              ...
              squares[shape_pos[i].y_c][shape_pos[i].x_c+1].in_use = false;
            }
      ...
      //Rotate Left
        ...
        if(action_ok == true){
          ...
          //Check for collision
          for(int i=0; i<shape_pos.length; i++){
            if(squares[shape_pos[i].y_c][shape_pos[i].x_c].in_use == true){
              //temp_state = false;
              //game_timer.start();
              //return;
              rotate_ok = false;
          ...
          if(temp_state == true){
            ...
            for(int i=0; i<prev_shape.length; i++){
              ...
              squares[prev_shape[i].y_c][prev_shape[i].x_c].in_use = false;
            }
      ...
      //Hard Drop
        ...
        if(action_ok == true){
          ...
          for(int i=0; i<shape_pos.length; i++){
            squares[shape_pos[i].y_c][shape_pos[i].x_c].setBackground(colors[prev_rand]);
            squares[shape_pos[i].y_c][shape_pos[i].x_c].in_use = true;
            action_ok = false;
          }
